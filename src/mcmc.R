...
# 
# save(m1, file = "results/models/mcmc.Rdata")

load(file = "results/models/mcmc.Rdata")
...
summary(m1)$solutions %>%
  write.csv("results/models/summary.csv", row.names = FALSE, fileEncoding = "latin1")
...
data.frame(
  ...
  min = coda::HPDinterval(lambda)[, 1] %>% round(2),
  max = coda::HPDinterval(lambda)[, 2] %>% round(2)) %>%
  write.csv("results/models/phylo-signal.csv", row.names = FALSE, fileEncoding = "latin1")
...
summary(m1)$Gcovariances %>%
  write.csv("results/models/random-effects.csv", row.names = FALSE, fileEncoding = "latin1")
...
ggsave(fig, file = "results/figures/mcmc.png", bg = "white",
